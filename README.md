...
Pour concevoir TodoList j’ai besoin d’une liste de Todo (List<Todo>), du getter sur cette liste qui va retourner une copie de la liste, des méthodes pour ajouter et supprimer les todo, et enfin de deux méthodes pour obtenir les Todo à faire ou ceux terminés.
...
La conception DDD est très moderne. Mon objectif est ici de n’en montrer qu’une sous partie. J’ai choisi d’illustrer trois concepts : Service, Value Object et Entity.

Le postulat est le suivant, certes tout est objet, mais tous les objets ne sont pas les mêmes.
Donc un Value Object est un objet qui représente une valeur et même une constante. L’exemple le plus utilisé est celui du point GPS avec la latitude et la longitude. Chaque point GPS est défini par sa latitude et sa longitude. Du coup, aucun intérêt à changer les valeurs (pas de setter).
...
Enfin, TodoList est une Entity. Il contient l’ensemble des Todo et surtout les méthodes permettant de dire qu’un Todo est fait (regardez le code, je supprime le Todo « à faire » et le remplace par un nouveau Todo « fait »).
...
Avec : todoList.didIt(todo1)
C’est clair, c’est TodoList qui est responsable du changement d’état de ses Todo.
